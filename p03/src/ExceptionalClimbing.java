...
//
// Title:    Climbing abstraction with exceptions
...
public class ExceptionalClimbing {
    ...
    public static int sendClimb(String[] send, int numSend, String grade) throws DataFormatException {
        ...
        if (numSend >= send.length)
            throw new IllegalArgumentException("cannot add new value to full length " + send.length + " array"); // it's
                                                                                                                 // full
        ...
        // validate positive array index
        if (numSend < 0){
            throw new DataFormatException("invalid oversize array");
        }

        // validate oversize array indices
    ...
    public static int failClimb(String[] fail, int numFail, String grade) throws DataFormatException {
        ...
        if (numFail == fail.length)
            throw new IllegalArgumentException("cannot add new value to full length " + fail.length + " array"); // it's full
    ...
    public static String getStats(String[] send, int numSend, String[] fail, int numFail, int historyLength) {
        ...
        if ((numSend == 0 || send.length == 0) && (numFail == 0 || send.length == 0)){
